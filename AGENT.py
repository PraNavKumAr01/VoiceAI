...
def get_llm_response(transcript):
    ...
    """
    search_result = searchTool.run(transcript, verbose = True)
    prompt = ChatPromptTemplate.from_messages([("human", template + "Web Search Results : {search_result}" +" User Query : {snippet}")])
    chain = prompt | llm
    answer = chain.invoke({"search_result" : search_result ,"snippet": transcript})
